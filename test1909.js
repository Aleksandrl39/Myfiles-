(function () {
    ...
    function getAlbumCover(artistName) {
        var url = `${iTunesApiUrl}?term=${encodeURIComponent(artistName)}&entity=album&limit=1`;
        return fetchData(url).then(data => data?.results?.[0]?.artworkUrl100 || "https://via.placeholder.com/100");
    }
    ...
    function createSearchPage() {
        ...
        function renderList(items) {
            ...
            items.forEach(item => {
                ...
                getAlbumCover(item.Name.split("-")[0].trim()).then(imageUrl => {
                    img.src = imageUrl;
                });
            });
        }
