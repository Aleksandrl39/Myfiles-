(function () {
    ...
    return fetch(url)
        ...
        .then(data => {
            if (url.includes("torrent")) { // Проверяем, что это запрос к API торрентов
                // Выводим JSON прямо на экран
                var debugContainer = document.createElement("pre");
                debugContainer.style.color = "yellow";
                debugContainer.style.backgroundColor = "black";
        ...
        .catch(error => {
            document.body.innerHTML = "<p>Ошибка API: " + error + "</p>";
            return null;
        });
}
    ...
    function getAlbums(artistName) {
        ...
        return fetchData(url).then(data => Array.isArray(data) ? data : (data?.results || []));
    }
    ...
    return fetchData(url).then(data => {
        document.body.innerHTML = "<pre>" + JSON.stringify(data, null, 2) + "</pre>"; // ВРЕМЕННЫЙ ВЫВОД ДАННЫХ
        return data?.results || [];
    });
}
