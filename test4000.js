(function () {
    ...
    async function searchTorrents(query) {
        ...
        return {
            RuTracker: data?.RuTracker || [],
            Rutor: data?.Rutor || [],
            NNMClub: data?.NNMClub || [],
            Kinozal: data?.Kinozal || []
        };
    }

    function getDetailsUrl(tracker, id) {
        return `${torrentDetailsApiUrl}${tracker.toLowerCase()}?query=${id}`;
    }

    async function getPosterById(tracker, id) {
        var url = getDetailsUrl(tracker, id);
        const data = await fetchData(url);
        return data?.[0]?.Poster || "https://via.placeholder.com/200";
    }

    async function getMagnetById(tracker, id) {
        var url = getDetailsUrl(tracker, id);
        const data = await fetchData(url);
    ...
    function createSearchPage() {
        ...
        function renderList(results) {
            ...
            // Объединяем результаты со всех трекеров
            Object.keys(results).forEach(tracker => {
                results[tracker].forEach(item => {
            ...
            allItems.forEach(item => {
                ...
                div.addEventListener("click", async () => {
                    const magnetLink = await getMagnetById(item.tracker, item.Id);
                    if (magnetLink) {
                ...
                // Подгружаем обложку
                getPosterById(item.tracker, item.Id).then(imageUrl => {
                    img.src = imageUrl;
                });
            });
        }
