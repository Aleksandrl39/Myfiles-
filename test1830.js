(function(){
    ...
    var torrentApiUrl = "http://127.0.0.1:8009/api/v1/all/search"; // API для поиска торрентов
    ...
    function searchTorrents(albumName) {
        var url = `${torrentApiUrl}?query=${encodeURIComponent(albumName)}`;
        return fetchData(url).then(data => data?.results || []);
    }
                ...
                renderList(artists, "artist", artist => {
                    ...
                    getAlbums(artist.artistName).then(albums => {
                        renderList(albums, "album", album => {
                            ...
                            searchTorrents(album.collectionName).then(torrents => {
