(function () {
    ...
    function getDetailsById(id) {
        ...
        return fetchData(url).then(data => ({
            poster: data?.RuTracker?.[0]?.Poster || "https://via.placeholder.com/200",
            magnet: data?.RuTracker?.[0]?.Magnet || ""
        }));
    }
        ...
        function renderList(items) {
            ...
            items.forEach(item => {
                ...
                div.addEventListener("click", () => {
                    getDetailsById(item.Id).then(data => {
                        window.open(data.magnet, "_blank");
                    });
                });
