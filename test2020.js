(function () {
    ...
    function getPosterById(id) {
        ...
        return fetchData(url).then(data => data?.RuTracker?.[0]?.Poster || "https://via.placeholder.com/200");
    }

    function getMagnetById(id) {
        var url = `${torrentDetailsApiUrl}${id}`;
        return fetchData(url).then(data => data?.RuTracker?.[0]?.Magnet || "");
    }
        ...
        function renderList(items) {
            ...
            items.forEach(item => {
                ...
                div.addEventListener("click", () => {
                    getMagnetById(item.Id).then(magnetLink => {
                        if (magnetLink) {
