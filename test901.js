(function () {
    ...
    var logContainer; // Делаем глобальной
    ...
    function searchTorrents(query) {  
        ...
        return fetch(url)  
            .then(response => response.json())  
            .then(data => {  
                logContainer.innerHTML = `<pre>${JSON.stringify(data, null, 2)}</pre>`; // Показываем JSON  
                return data?.RuTracker || [];  
            })  
            .catch(error => {  
                logContainer.innerHTML = `<p style="color:red;">Ошибка API: ${error}</p>`;  
                return [];  
            });  
    }
    ...
    function createSearchPage() {  
        ...
        function renderList(items) {
            ...
            items.forEach(item => {
                ...
                img.src = "https://via.placeholder.com/100"; // Заглушка
                ...
                div.addEventListener("click", () => {
                    window.open(item.Torrent, "_blank"); // Открываем торрент
                });

                resultsContainer.appendChild(div);

                // Загружаем обложку из Last.fm
