                ...
                categories.forEach(category => {
                    if (json.hasOwnProperty(category) && Array.isArray(json[category])) {
                        json[category].forEach(item => {
                            results.push({
                                id: item.Id,
                                name: item.Name || 'Без названия', // Используем name вместо title
                                author: 'Автор неизвестен', // Добавляем поле author
                                url: item.Url || '', // Ссылка на элемент
                                status: 1, // По умолчанию включено
                                premium: false, // По умолчанию не премиум
                                descr: `Размер: ${item.Size || 'неизвестно'}. Сиды: ${item.Seeds || 0}, Пиры: ${item.Peers || 0}`, // Используем descr вместо description
