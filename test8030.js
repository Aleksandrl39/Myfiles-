    ...
    function search(params = {}, oncomplite){
        ...
        network.silent(url, (json)=>{
            ...
            if (json && typeof json === 'object') {
                // Если API возвращает сообщение об отсутствии совпадений, возвращаем пустой результат
                ...
                categories.forEach(category => {
                    if (json.hasOwnProperty(category) && Array.isArray(json[category])) {
                        json[category].forEach(item => {
                            if (item.Url || item.Torrent) { // Если есть хотя бы одна ссылка, добавляем объект
                                results.push({
                                    ...
                                    results: []  // Добавлено для совместимости с Lampa (из results.js)
                                });
                            }
                        });
                    }
